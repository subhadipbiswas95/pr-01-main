...
export default class Home extends Component {
  ...
  componentDidMount(){
    fetch("http://localhost:3000/asset/data.json").then(resp=>resp.json()).then(resp=>this.setState({post:resp}))
  }
  render() {
    return (
      <div>{this.state.post.map(post=><div>{post.CardImgSrc}</div>)}</div>
    )
  }
}
